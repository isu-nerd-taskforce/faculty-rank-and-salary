...
ScholarStats = function(First,Last){
  ...
  profile.url = readnodes[19]
  profile.url = substr(profile.url,36,47)
  scholarid = profile.url
  num_article = get_num_articles(profile.url)
  top_article = get_num_top_journals(profile.url,top.journals)
  return(c(scholarid,num_article,top_article))
}

#for(i in 1:nrow(polisci.profs)){
for(i in 1:25){
  if(i==1){
    add.mat = c(tryCatch(ScholarStats(polisci.profs$First[i],polisci.profs$Last[i]), error=function(e){}))
  }
  else{
    add.mat = rbind(add.mat,c(tryCatch(ScholarStats(polisci.profs$First[i],polisci.profs$Last[i]), error=function(e){return(c(-1,-1,-1))})))
  }
}

final = cbind(polisci.profs,add.mat)
colnames(final) = list("First","Last","ScholarID","num_article","top_article")
